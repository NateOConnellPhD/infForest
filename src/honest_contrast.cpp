...
// [[Rcpp::export]]
List honest_predict_contrast(
    List forest,
    NumericMatrix X_num,
    NumericVector y_honest,
    IntegerVector honest_idx,
    IntegerVector cont_cols,
    NumericVector cont_va,
    NumericVector cont_vb
) {
    // Hybrid Case 1 / Case 2 for continuous predictor contrasts.
    // Estimates f_0(X_i, X_j=a) - f_0(X_i, X_j=b) at analyst-specified a,b.
    //
    // Case 1: Tree split on X_j on obs i's path.
    //   Route (X_i, X_j=a) and (X_i, X_j=b) to different leaves.
    //   Contrast = honest_leaf_mean(leaf_a) - honest_leaf_mean(leaf_b).
    //
    // Case 2: Tree did NOT split on X_j. Obs i in leaf ell.
    //   Force midpoint split at c = (a+b)/2.
    //   H+ = honest obs in ell with X_j >= c, H- = X_j < c.
    //   Contrast = [(Ybar_H+ - Ybar_H-) / (Xbar_j_H+ - Xbar_j_H-)] * (a - b)
    //   Per-leaf slope × nominal span. Works for both continuous and binary Y.
    ...
    std::vector<int> ccols(cont_cols.begin(), cont_cols.end());
    std::vector<double> cva(cont_va.begin(), cont_va.end());
    std::vector<double> cvb(cont_vb.begin(), cont_vb.end());
    ...
    for (int b = 0; b < B; b++) {
        ...
        // Route all obs to natural leaves
        std::vector<int> leaf_id(n);
        for (int i = 0; i < n; i++) {
            int node = 0;
            while (lc[node] != 0 || rc[node] != 0) {
        ...
        // Compute honest leaf means (for Case 1)
        std::unordered_map<int, double> leaf_sum;
        std::unordered_map<int, int> leaf_cnt;
        for (int i = 0; i < n; i++) {
            if (!is_honest[i]) continue;
            double yi = y_ptr[i];
            if (ISNA(yi)) continue;
            leaf_sum[leaf_id[i]] += yi;
            leaf_cnt[leaf_id[i]]++;
        }
        std::unordered_map<int, double> leaf_mean;
        for (auto& kv : leaf_cnt) {
            if (kv.second > 0)
                leaf_mean[kv.first] = leaf_sum[kv.first] / kv.second;
        }
        ...
        for (int m = 0; m < n_cont; m++) {
            ...
            double va = cva[m];
            double vb = cvb[m];
            double midpt = (va + vb) / 2.0;
            double span = va - vb;

            // DFS: label nodes with first ancestor that splits on this col
            std::vector<int> xj_anc(n_nodes, -1);
            {
                struct DE { int node; int anc; };
                std::vector<DE> stk;
            ...
            // --- Case 2: forced midpoint split for non-diverging obs ---
            // Accumulate honest Y and X_j by region (leaf or pooled-downstream)
            struct Sums {
                double sy_hi=0, sy_lo=0, sx_hi=0, sx_lo=0;
                int nhi=0, nlo=0;
            };
            std::unordered_map<int, Sums> c2_sums;
            ...
            for (int i = 0; i < n; i++) {
                ...
                int anc = xj_anc[leaf_id[i]];
                if (anc < 0) {
                    // Case 2: no X_j split ancestor
                    auto& s = c2_sums[leaf_id[i]];
                    if (xi >= midpt) { s.sy_hi += yi; s.sx_hi += xi; s.nhi++; }
                    else              { s.sy_lo += yi; s.sx_lo += xi; s.nlo++; }
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (anc >= 0) {
                    ...
                    while (lc[node_a] != 0 || rc[node_a] != 0) {
                        double xval = (sv[node_a] == col) ? va : X_ptr[i + n * sv[node_a]];
                        node_a = (xval <= sval[node_a]) ? lc[node_a] : rc[node_a];
                    }
                    int node_b = 0;
                    while (lc[node_b] != 0 || rc[node_b] != 0) {
                        double xval = (sv[node_b] == col) ? vb : X_ptr[i + n * sv[node_b]];
                        node_b = (xval <= sval[node_b]) ? lc[node_b] : rc[node_b];
                    }
                    if (node_a != node_b) {
                        auto it_a = leaf_mean.find(node_a);
                    ...
                    // If same leaf (both queries on same side of split), skip — no info
                } else {
                    // Case 2: no X_j split. Use forced midpoint contrast.
